...
import { activeTeams, computeStandings, formatDate, generatePlayoffs, groupStageProgress, hasPlayoffs, syncPlayoffBracket } from "@/lib/tournament";
import type { Match, MatchStage, MatchStatus, Team, TournamentData, Player } from "@/lib/types";

type Tab = "general" | "teams" | "matches" | "playoffs" | "rules";
...
export function AdminPanel() {
  ...
  const groupProgress = useMemo(() => groupStageProgress(draft), [draft]);
  const playoffsReady = groupProgress.complete;
  const playoffsCreated = useMemo(() => hasPlayoffs(draft), [draft]);
  const playoffMatches = useMemo(() => draft.matches.filter((match) => match.playoffSlot), [draft.matches]);
  ...
  const handleSave = async () => {
    setSaving(true);
    setSaveError(null);
    const normalized = syncPlayoffBracket(draft);
    setDraft(normalized);
  ...
  const deleteMatch = (id: string) => {
    const target = draft.matches.find((match) => match.id === id);
    if (target?.playoffSlot) {
      window.alert("Esta partida pertenece a la llave. Usa ‘Resetear Playoffs’ desde la sección Playoffs si necesitas rehacerla.");
      return;
    }
  ...
  const publishImmediate = async (next: TournamentData) => {
    const normalized = syncPlayoffBracket(next);
    setDraft(normalized);
    setSaving(true);
    setSaveError(null);
    const result = await saveData(normalized);
  ...
  const handleGeneratePlayoffs = async () => {
    if (!playoffsReady || playoffsCreated) return;
    if (!window.confirm("Se generará la llave usando las posiciones finales de los grupos. ¿Continuar?")) return;
    const next = generatePlayoffs(draft);
    await publishImmediate(next);
    setTab("playoffs");
  };

  const handleResetPlayoffs = async () => {
    if (!playoffsCreated) return;
    if (!window.confirm("¿Resetear toda la llave de Playoffs? Se eliminarán los partidos de Upper/Lower/Gran Final, pero NO la fase de grupos.")) return;
    const next = { ...draft, matches: draft.matches.filter((match) => !match.playoffSlot) };
    await publishImmediate(next);
  };
    ...
    <main className="admin-page">
      <aside className="admin-sidebar">
        ...
        <nav>
          ...
          <button className={tab === "playoffs" ? "active" : ""} onClick={() => setTab("playoffs")}><Icon name="trophy"/>Playoffs</button>
      ...
      <section className="admin-content">
        <header className="admin-topbar">
          <div><span>ADMINISTRACIÓN</span><h1>{tab === "general" ? "Configuración del torneo" : tab === "teams" ? "Equipos participantes" : tab === "matches" ? "Partidas y resultados" : tab === "playoffs" ? "Playoffs" : "Reglamento"}</h1></div>
            ...
            <div className="admin-match-bottom">
              <div className="admin-card match-edit-focus">
                {selectedMatch ? <>
                  ...
                  <div className="field-row"><Field label="Etapa"><select value={selectedMatch.stage} disabled={Boolean(selectedMatch.playoffSlot)} onChange={(e)=>updateMatch(selectedMatch.id,"stage",e.target.value as MatchStage)}><option>Todos contra todos</option><option>Upper</option><option>Lower</option><option>Final</option></select></Field><Field label="Llave"><input value={selectedMatch.playoffRound ?? "Partida manual"} disabled /></Field></div>
        ...
        {tab === "playoffs" && (
          <div className="admin-playoffs-layout">
            <div className="admin-card full playoff-generator-card">
              <div className="playoff-generator-main">
                <div>
                  <span className={`playoff-state-pill ${playoffsCreated ? "done" : playoffsReady ? "ready" : "pending"}`}>{playoffsCreated ? "GENERADOS" : playoffsReady ? "LISTO" : "PENDIENTE"}</span>
                  <h2>Generar Playoffs</h2>
                  <p>{playoffsCreated ? "La llave ya está activa y se muestra en la página pública." : playoffsReady ? "Todos los cruces de grupos finalizaron. Ya puedes generar la llave." : `Fase de grupos: ${groupProgress.completed} / ${groupProgress.total} cruces finalizados.`}</p>
                </div>
                {!playoffsCreated ? <button className="primary-button playoff-generate-button" disabled={!playoffsReady || saving} onClick={()=>void handleGeneratePlayoffs()}><Icon name="trophy"/>Generar Playoffs</button> : <button className="ghost-button playoff-reset-button" disabled={saving} onClick={()=>void handleResetPlayoffs()}>Resetear Playoffs</button>}
              </div>
              <div className="group-progress-track"><i style={{ width: `${groupProgress.total ? (groupProgress.completed / groupProgress.total) * 100 : 0}%` }} /></div>
              <div className="group-pair-status-grid">
                {groupProgress.pairs.map((pair) => <div key={`${pair.group}-${pair.teamA.id}-${pair.teamB.id}`} className={pair.finished ? "done" : "pending"}><span>Grupo {pair.group}</span><b>{pair.teamA.shortName} vs {pair.teamB.shortName}</b><small>{pair.finished ? "Finalizado" : "Pendiente"}</small></div>)}
              </div>
            </div>

            {playoffsCreated ? <div className="admin-card full playoff-admin-bracket">
              <div className="admin-card-heading"><div><h2>Llave activa</h2><p>Los ganadores y perdedores avanzan automáticamente al finalizar cada serie.</p></div><Link className="secondary-button" href="/#playoffs" target="_blank">Ver llave pública</Link></div>
              <div className="playoff-admin-grid">
                {(["UF","LR1A","LR1B","LR2","LF","GF"] as const).map((slot) => {
                  const match = playoffMatches.find((item) => item.playoffSlot === slot);
                  if (!match) return null;
                  const a = teams.find((team)=>team.id===match.teamA);
                  const b = teams.find((team)=>team.id===match.teamB);
                  return <article className={`playoff-admin-match slot-${slot.toLowerCase()}`} key={slot}>
                    <header><span>{match.playoffRound}</span><b>BO{match.bestOf}</b></header>
                    <div className="playoff-admin-team"><strong>{a?.name ?? "Por definir"}</strong><b>{match.status === "Programado" ? "—" : match.scoreA}</b></div>
                    <div className="playoff-admin-team"><strong>{b?.name ?? "Por definir"}</strong><b>{match.status === "Programado" ? "—" : match.scoreB}</b></div>
                    <div className="field-row"><Field label="Fecha"><input type="date" value={match.date} onChange={(e)=>updateMatch(match.id,"date",e.target.value)}/></Field><Field label="Hora"><input type="time" value={match.time} onChange={(e)=>updateMatch(match.id,"time",e.target.value)}/></Field></div>
                    <button className="secondary-button" onClick={()=>{setSelectedMatchId(match.id);setTab("matches")}}>Administrar resultado</button>
                  </article>;
                })}
              </div>
            </div> : null}
