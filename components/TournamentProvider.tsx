...
import type { Match, MatchStage, MatchStatus, Player, PlayoffBracket, PlayoffSlot, Team, TeamGroup, TournamentData } from "@/lib/types";
...
function validBracket(value: unknown): PlayoffBracket | undefined {
  if (value === "upper" || value === "lower" || value === "final") return value;
  return undefined;
}

function validPlayoffSlot(value: unknown): PlayoffSlot | undefined {
  if (value === "UF" || value === "LR1A" || value === "LR1B" || value === "LR2" || value === "LF" || value === "GF") return value;
  return undefined;
}
...
async function fetchTournamentData(): Promise<TournamentData> {
  ...
  const matches: Match[] = (matchesRes.data ?? []).map((row) => ({
    ...
    bracket: validBracket(row.bracket),
    playoffRound: row.playoff_round ? String(row.playoff_round) : undefined,
    playoffSlot: validPlayoffSlot(row.playoff_slot),
    winnerTo: validPlayoffSlot(row.winner_to),
    loserTo: validPlayoffSlot(row.loser_to),
  }));
...
async function persistTournamentData(next: TournamentData): Promise<TournamentData> {
  ...
  const mapMatch = (match: Match) => ({
    ...
    stage: match.stage,
    status: match.status,
    group_name: match.stage === "Todos contra todos" ? (next.teams.find((team) => team.id === match.teamA)?.group ?? null) : null,
    stream_url: match.streamUrl ?? null,
    bracket: match.bracket ?? null,
    playoff_round: match.playoffRound ?? null,
    playoff_slot: match.playoffSlot ?? null,
    winner_to: match.winnerTo ?? null,
    loser_to: match.loserTo ?? null,
  });
