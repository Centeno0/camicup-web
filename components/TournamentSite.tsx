...
import { activeTeams, computeStandings, formatDate, formatMoney, hasPlayoffs, playoffMatch } from "@/lib/tournament";
...
function matchContext(match: Match, team?: Team) {
  return match.playoffRound ?? (team ? `Grupo ${team.group}` : match.stage);
}
...
function FeaturedMatch({ match, teams }: { match: Match; teams: Team[] }) {
  ...
  return (
    <article className={`final-main-match ${live ? "is-live" : ""}`}>
      ...
      <MatchTeam team={b} />
      <div className="final-match-format"><strong>{matchContext(match, a)} · BO{match.bestOf}</strong><span>{match.stage}</span></div>
    </article>
  );
}
...
function UpcomingMatches({ matches, teams }: { matches: Match[]; teams: Team[] }) {
  const live = [...matches]
    .filter((match) => match.status === "En vivo" && match.teamA && match.teamB)
    .sort((a, b) => matchSortValue(a).localeCompare(matchSortValue(b)));
  const scheduled = [...matches]
    .filter((match) => match.status === "Programado" && match.teamA && match.teamB && match.time !== "00:00")
    .sort((a, b) => matchSortValue(a).localeCompare(matchSortValue(b)));
            ...
            {upcoming.map((match) => {
              ...
              return (
                <article className="final-mini-match" key={match.id}>
                  ...
                  <small>{matchContext(match, a)} · BO{match.bestOf}</small>
                </article>
              );
            })}
          </div>
...
function BracketTeam({ team, placeholder, score, showScore }: { team?: Team; placeholder: string; score: number; showScore: boolean }) {
  return (
    <div className={`playoff-bracket-team ${team ? "resolved" : "pending"}`}>
      {team ? <TeamLogo team={team} size={46} /> : <span className="playoff-question">?</span>}
...
function PlayoffSeries({ match, teams, placeholderA, placeholderB, className = "" }: { match?: Match; teams: Team[]; placeholderA: string; placeholderB: string; className?: string }) {
  if (!match) return null;
  const a = teams.find((team) => team.id === match.teamA);
  const b = teams.find((team) => team.id === match.teamB);
  const showScore = match.status === "En vivo" || match.status === "Finalizado";
  return (
    <article className={`playoff-series ${match.playoffSlot === "GF" ? "grand-final" : ""} ${className}`}>
      <header>
        <span>{match.playoffRound}</span>
        <b>BO{match.bestOf}</b>
      </header>
      <BracketTeam team={a} placeholder={placeholderA} score={match.scoreA} showScore={showScore} />
      <div className="playoff-vs">VS</div>
      <BracketTeam team={b} placeholder={placeholderB} score={match.scoreB} showScore={showScore} />
      {match.status !== "Programado" ? <small className={`playoff-series-status ${match.status === "En vivo" ? "live" : "done"}`}>{match.status}</small> : null}
    </article>
  );
}

function PlayoffsSection({ data, teams }: { data: ReturnType<typeof useTournament>["data"]; teams: Team[] }) {
  if (!hasPlayoffs(data)) return null;
  const uf = playoffMatch(data, "UF");
  const lr1a = playoffMatch(data, "LR1A");
  const lr1b = playoffMatch(data, "LR1B");
  const lr2 = playoffMatch(data, "LR2");
  const lf = playoffMatch(data, "LF");
  const gf = playoffMatch(data, "GF");

  return (
    <section className="final-section playoffs-section" id="playoffs">
      <div className="playoffs-title"><span>PLAYOFFS</span></div>
      <div className="playoffs-board">
        <svg className="playoff-connectors" viewBox="0 0 1200 560" preserveAspectRatio="none" aria-hidden="true">
          <defs><linearGradient id="playoffLine" x1="0" x2="1"><stop offset="0" stopColor="#ff43ad"/><stop offset="1" stopColor="#a45cff"/></linearGradient></defs>
          <path d="M276 284 H340 V360 H416" />
          <path d="M276 446 H340 V360 H416" />
          <path d="M584 360 H668" />
          <path d="M836 360 H930" />
          <path d="M746 126 H876 V206 H930" />
          <path d="M746 126 H858 V360 H668" className="secondary-path" />
        </svg>
        <div className="playoff-slot playoff-upper"><PlayoffSeries match={uf} teams={teams} placeholderA="1.º Grupo A" placeholderB="1.º Grupo B" /></div>
        <div className="playoff-slot playoff-lr1a"><PlayoffSeries match={lr1a} teams={teams} placeholderA="2.º Grupo A" placeholderB="3.º Grupo B" /></div>
        <div className="playoff-slot playoff-lr1b"><PlayoffSeries match={lr1b} teams={teams} placeholderA="3.º Grupo A" placeholderB="2.º Grupo B" /></div>
        <div className="playoff-slot playoff-lr2"><PlayoffSeries match={lr2} teams={teams} placeholderA="Ganador LR1 · Partida 1" placeholderB="Ganador LR1 · Partida 2" /></div>
        <div className="playoff-slot playoff-lf"><PlayoffSeries match={lf} teams={teams} placeholderA="Ganador Lower Round 2" placeholderB="Perdedor Upper Bracket" /></div>
        <div className="playoff-slot playoff-gf"><PlayoffSeries match={gf} teams={teams} placeholderA="Ganador Upper Bracket" placeholderB="Ganador Lower Final" /></div>
      </div>
    </section>
  );
}
    ...
    <main className="accurate-site final-public-site">
      ...
      <section className="accurate-hero" id="inicio">
        <div className="accurate-copy">
          ...
          <div className="accurate-organizers">Organizado por <strong>Spektra68</strong> y <strong>Dylan</strong></div>
      ...
      <PlayoffsSection data={data} teams={teams} />
